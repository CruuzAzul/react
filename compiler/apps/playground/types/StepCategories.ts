...
export const getCategoryColor = (category: string): string | null => {
  ...
  if (OPTIMIZATION_AND_NORMALIZATION_CATEGORIES.has(category as OptimizationAndNormalizationCategory)) {
    return CATEGORY_CLASSES_COLORS.OPTIMIZATION_AND_NORMALIZATION;
  }
  if (STATIC_ANALYSIS_AND_TYPE_INFERENCE_CATEGORIES.has(category as StaticAnalysisAndTypeInferenceCategory)) {
    return CATEGORY_CLASSES_COLORS.STATIC_ANALYSIS_AND_TYPE_INFERENCE;
  }
  if (REACTIVE_OPTIMIZATION_HIR_CATEGORIES.has(category as ReactiveOptimizationHIRCategory)) {
    return CATEGORY_CLASSES_COLORS.REACTIVE_OPTIMIZATION_HIR;
  }
  if (REACTIVE_OPTIMIZATION_REACTIVE_FUNCTION_CATEGORIES.has(category as ReactiveOptimizationReactiveFunctionCategory)) {
...
const OPTIMIZATION_AND_NORMALIZATION_CATEGORIES = new Set(Object.values(OptimizationAndNormalizationCategory));
const STATIC_ANALYSIS_AND_TYPE_INFERENCE_CATEGORIES = new Set(Object.values(StaticAnalysisAndTypeInferenceCategory));
const REACTIVE_OPTIMIZATION_HIR_CATEGORIES = new Set(Object.values(ReactiveOptimizationHIRCategory));
const REACTIVE_OPTIMIZATION_REACTIVE_FUNCTION_CATEGORIES = new Set(Object.values(ReactiveOptimizationReactiveFunctionCategory));
